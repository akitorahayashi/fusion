...
fn llm_ollama_health_sends_inference_request() {
    ...
    cli::handle_health_single(ServiceType::Ollama).expect("health should succeed");
...
fn llm_mlx_health_sends_inference_request() {
    ...
    let stub_thread = thread::spawn(move || {
        ...
        // Verify minimal inference payload with MLX model
        assert_eq!(json["model"], "mlx-community/Llama-3.2-3B-Instruct-4bit");
        assert_eq!(json["max_tokens"], 10);
        assert_eq!(json["stream"], false);
        let messages = json["messages"].as_array().expect("messages should be an array");
        assert_eq!(messages.len(), 1);
    ...
    cli::handle_health_single(ServiceType::Mlx).expect("health should succeed");
